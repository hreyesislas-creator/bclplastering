...
export default async function ServiceDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const [s, settings, allServices] = await Promise.all([
  ...
  const heroManifest = images.services[s.slug as keyof typeof images.services];
  const heroUrl = s.imageUrl || (heroManifest?.enabled ? heroManifest.src : null);
  const heroAlt = s.imageUrl
    ? s.title
    : heroManifest?.enabled
      ? heroManifest.alt
      : s.title;
  const showHero = Boolean(heroUrl);
  const ld = serviceSchema({
    service: {
      slug: s.slug,
      title: s.title,
      description: s.description || s.short,
      startingFrom: s.startingFrom ?? undefined,
    },
    settings,
    baseUrl: env.siteUrl,
  });
